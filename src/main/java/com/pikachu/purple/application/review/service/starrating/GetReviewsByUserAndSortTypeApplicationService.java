...
class GetReviewsByUserAndSortTypeApplicationService implements
    ...
    public Result invoke(String sortType) {
        ...
        switch (getSortType) {
            case LIKED:
                starRatings = starRatingRepository.findAllOrderByLikeCountDesc(userId);
                break;
            case LATEST:
                starRatings = starRatingRepository.findAllByUserId(userId);
            ...
            case STAR_RATING_HIGH:
                starRatings = starRatingRepository.findAllOrderByScoreDesc(userId);
                break;
            case STAR_RATING_LOW:
                starRatings = starRatingRepository.findAllOrderByScoreAsc(userId);
                break;
            default:
                break;
        }
